...
const stageColor=g=>({'ID/Qualify':B.silver,'Gate A':B.supernova,'Gate B':B.force,'Gate C':B.sky,'Proposal':B.twilight,'Won':B.refraction,'Lost':'#555','No Bid':'#444'}[g]||B.silver);
const threatColor=t=>t==='High'?B.twilight:t==='Medium'?B.supernova:B.refraction;
const priorityC=p=>p==='High'?B.twilight:p==='Medium'?B.supernova:B.silver;
const gateColor=s=>s==='Complete'?B.refraction:s==='In Progress'?B.supernova:s==='Upcoming'?B.sky:B.border;
const influenceC=i=>i==='High'?B.twilight:i==='Med'?B.supernova:B.silver;
const cparsColor=r=>({Exceptional:B.refraction,'Very Good':B.sky,Satisfactory:B.supernova,Marginal:B.twilight,Unsatisfactory:'#555'}[r]||B.silver);
const fmtBytes=b=>b>1048576?`${(b/1048576).toFixed(1)} MB`:b>1024?`${(b/1024).toFixed(0)} KB`:`${b} B`;

/* TCV Smart Format */
const parseTCVNum=raw=>{
  if(!raw)return 0;
...
const fileIsImage=t=>t?.startsWith('image/');

const STAGES=['ID/Qualify','Gate A','Gate B','Gate C','Proposal','Won','Lost','No Bid'];
const COMP_SIZES=['Large Business','Small Business','SDB','WOSB','SDVOSB','HUBZone','VOSB','8(a)','Other'];
...
function exportToDoc(text,title,subtitle){
